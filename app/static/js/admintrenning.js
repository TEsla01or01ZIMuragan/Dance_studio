  ...
  eventsList.innerHTML = items.length ? items.map((item) => `
    <article class="admin-list-item">
      <div>
        ...
        <span>${new Date(item.event_date).toLocaleString('ru-RU')}${item.location ? ' · ' + escapeHtml(item.location) : ''}</span>
      </div>
...
async function loadSchedule() {
  ...
  scheduleList.innerHTML = items.length ? items.map((item) => `
    <article class="admin-list-item">
      <div>
        <strong>${escapeHtml(item.title)}</strong>
        <p>${escapeHtml(item.weekday)}, ${escapeHtml(item.start_time)} - ${escapeHtml(item.end_time)}</p>
        <span>${escapeHtml(item.teacher)}${item.hall ? ' · ' + escapeHtml(item.hall) : ''}${item.level ? ' · ' + escapeHtml(item.level) : ''}</span>
      </div>
      <div class="admin-list-actions">
